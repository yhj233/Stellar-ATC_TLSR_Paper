...
// Direct port of L3N's superior display driver
...
#define EPD_BW213_M3N_test_pattern 0xA5
_attribute_ram_code_ uint8_t EPD_BW213_M3N_detect(void)
{
    ...
    for (i = 0; i < 153; i++)
    {
        EPD_WriteData(EPD_BW213_M3N_test_pattern);
    }
    EPD_WriteCmd(0x33);
    for (i = 0; i < 153; i++)
    {
        if(EPD_SPI_read() != EPD_BW213_M3N_test_pattern)
            return 0;
    }
    return 1;
}
...
_attribute_ram_code_ uint8_t EPD_BW213_M3N_read_temp(void)
{
    ...
    // Set RAM Y- Address Start/End (250 pixels: 0x002E to 0x0128)
    EPD_WriteCmd(0x45);
    EPD_WriteData(0x28);
	EPD_WriteData(0x01);
	EPD_WriteData(0x2E);
...
_attribute_ram_code_ uint8_t EPD_BW213_M3N_Display_BWR(unsigned char *image, unsigned char *red_image, int size, uint8_t full_or_partial) {
    if (red_image == NULL) {
        return EPD_BW213_M3N_Display(image, size, full_or_partial);
    }
